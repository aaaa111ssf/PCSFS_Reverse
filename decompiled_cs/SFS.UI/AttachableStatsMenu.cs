...
public class AttachableStatsMenu : MonoBehaviour
{
	...
	public OpenTracker Open_DrawPartShop(Func<bool> hasControl, Part[] allParts, PartDrawSettings partDrawSettings, Func<Vector2> getScreenPosition, bool dontUpdateOnZoomChange, bool skipAnimation, Action onOpen = null, Action onClose = null)
	{
		onOpen = (Action)Delegate.Combine(onOpen, (Action)delegate
		{
			Part[] array = allParts;
		...
		return Open(hasControl, Draw, getScreenPosition, dontUpdateOnZoomChange, skipAnimation, onOpen, onClose);
		void Draw(StatsMenu statsMenu)
		{
			allParts[0].DrawPartStats(allParts, statsMenu, partDrawSettings);
			statsMenu.DrawSpace(-1004, 15f);
			statsMenu.DrawFullButton(-1005, Loc.main.Open_Shop_Menu_2, delegate
			{
				Base.sceneLoader.LoadHomeScene("PickGridButton");
			});
		}
		void OnPartDestroyed(Part _)
		{
