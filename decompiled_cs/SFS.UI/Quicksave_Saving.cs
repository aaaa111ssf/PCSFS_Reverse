...
public class Quicksave_Saving : I_SavingBase
{
	...
	void I_SavingBase.Save(string name)
	{
		...
		{
			MenuGenerator.AskOverwrite(() => Loc.main.File_Already_Exists.InjectField(Loc.main.Quicksave, "filetype", formatCapitalization: true), () => Loc.main.Overwrite_File.InjectField(Loc.main.Quicksave, "filetype", formatCapitalization: true), () => Loc.main.New_File.InjectField(Loc.main.Quicksave, "filetype", formatCapitalization: true), delegate
			{
				Save(name);
			}, delegate
			{
		...
		{
			paths.quicksavesPath.Create();
			WorldSave worldSave = getQuicksave();
			IFolder path = paths.GetQuicksavePath(finalName);
	...
	void I_SavingBase.Rename(string oldName, string newName)
	{
		if (!(oldName == newName))
		{
			...
			IFolder quicksavePath = paths.GetQuicksavePath(oldName);
			IFolder quicksavePath2 = paths.GetQuicksavePath(newName);
			if (quicksavePath.Exists())
			{
	...
	{
		paths.GetQuicksavePath(name).Delete();
