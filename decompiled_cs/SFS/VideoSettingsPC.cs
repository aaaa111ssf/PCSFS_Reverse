...
public class VideoSettingsPC : SettingsBase<VideoSettingsPC.Data>
{
	...
	{
		Init();
		Resolution item = resolutionList.FirstOrDefault((Resolution p) => (float)p.width == settings.currentResolution.x && (float)p.height == settings.currentResolution.y);
		videoResolutionDropdown.value = resolutionList.IndexOf(item, 0);
		TMP_Dropdown tMP_Dropdown = windowModeDropdown;
		tMP_Dropdown.value = settings.windowMode switch
		{
			0 => 0, 
			1 => 1, 
			3 => 2, 
