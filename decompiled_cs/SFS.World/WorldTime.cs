...
public class WorldTime : MonoBehaviour
{
	...
	public static bool CanTimewarp(bool showMsg, bool showSpeed, out bool isInWater)
	{
		isInWater = false;
		...
		if (main.realtimePhysics.Value)
		{
			if (value != null)
			{
				...
				return value.CanTimewarp(logger, showSpeed, out isInWater);
			}
			return false;
		}
		return true;
	...
	public void AccelerateTime()
	{
		...
		{
			if (CanTimewarp(showMsg: false, showSpeed: false, out var isInWater) && (!isInWater || timewarpIndex == MaxPhysicsIndex))
			{
				Accelerate_Rails();
			}
			...
			{
				CanTimewarp(showMsg: true, showSpeed: true, out var _);
			}
		}
	}
	...
	{
		if (!realtimePhysics && !CanTimewarp(showMsg: true, showSpeed: false, out var _))
		{
			StopTimewarp(showMsg: false);
		}
	}
