...
public class Rocket : Player, I_Physics
{
	...
	private bool CanUsePart(Part part)
	{
		...
		{
			MsgDrawer.main.Log(Loc.main.Cannot_Use_Part_While_Timewarping.InjectField(part.GetDisplayName(), "part", formatCapitalization: true));
			return false;
		}
		return true;
	}
	...
	public override bool CanTimewarp(I_MsgLogger logger, bool showSpeed, out bool isInWater)
	{
		bool flag = false;
		isInWater = floating;
		...
		if (location.Value.Radius < timewarpRadius_AscendDescend)
		{
			...
			if (location.velocity.Value.Mag_MoreThan(floating ? 3.0 : 0.1))
			{
				WorldTime.ShowCannotTimewarpMsg(floating ? Loc.main.Cannot_Timewarp_While_Moving_Water : Loc.main.Cannot_Timewarp_While_Moving_On_Surface, logger);
				return false;
			}
		}
	...
	private float GetTurnAxis(float torque, bool useStopRotation)
	{
		...
		if (useStopRotation && (bool)hasControl && !IsOnSurface)
		{
