...
public class WorldView : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (Application.isEditor && GameManager.main != null)
		{
			GameManager.main.world_Input.keysNode.AddOnKey(new KeybindingsPC.Key
			{
				key = KeyCode.V
			}, delegate
			{
				viewDistance.Value /= 1f + Time.deltaTime * 0.5f;
			});
			GameManager.main.world_Input.keysNode.AddOnKey(new KeybindingsPC.Key
			{
				key = KeyCode.B
	...
	private void Update()
	{
		...
		if (!(value is Rocket))
		{
			...
			{
				target = Mathf.Lerp(0.42f, 0.5f, Mathf.InverseLerp(0f, 100f, (float)value.location.Value.GetTerrainHeight(clampToWater: true)));
			}
		}
	...
	private void UpdatePostProcessing()
	{
		...
		PostProcessingModule.Key ambient = (data.hasPostProcessing ? data.postProcessing.Evaluate(num + viewDistance.Value) : new PostProcessingModule.Key(0f, 1.75f, 1f, 0f, 0.95f, 1.1f, 1f, 1f, 1f));
