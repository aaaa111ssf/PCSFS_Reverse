...
public class Astronaut_EVA : Player, I_Physics
{
	...
	public bool CanPickItselfUp
	{
		get
		{
			...
			{
				return rb2d.linearVelocity.magnitude < 0.2f;
			}
			return false;
		}
	}
	...
	private void OnFixedUpdate(Vector2 gravity)
	{
		...
		Double2 @double = WorldView.ToGlobalVelocity(rb2d.linearVelocity);
		...
		rb2d.linearVelocity = WorldView.ToLocalVelocity(@double);
	...
	public static void GetGroundRadius(Location location, out double groundAngleRadians, out double groundRadius)
	{
		...
		groundRadius = location.planet.Radius + location.planet.GetTerrainHeightAtAngle(position.AngleRadians, clampToWater: false);
