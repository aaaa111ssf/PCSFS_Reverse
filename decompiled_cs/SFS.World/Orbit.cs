...
public class Orbit : I_Path
{
	...
	private static Dictionary<int, Vector3[]> ellipseCache = new Dictionary<int, Vector3[]>();
	...
	{
		success = false;
		if (location.velocity.Mag_LessThan((location.planet.data.basics.gravity > 1.0 && location.Radius < location.planet.TimewarpRadius_Ascend) ? 10.0 : 0.1))
		{
			return null;
		}
	...
	public Vector3[] GetPoints(double fromTrueAnomaly, double toTrueAnomaly, int resolution, double scaleMultiplier)
	{
		if (ecc < 1.0)
		{
			return GetPoints_Ellipse_Optimized(fromTrueAnomaly, toTrueAnomaly, resolution, scaleMultiplier);
		}
	...
	public Vector3[] GetPoints_Ellipse_Optimized(double fromTrueAnomaly, double toTrueAnomaly, int resolution, double scaleMultiplier)
	{
		double num = Kepler.NormalizeAngle(fromTrueAnomaly);
		double num2 = Kepler.NormalizeAngle(toTrueAnomaly);
		bool flag = fromTrueAnomaly == toTrueAnomaly;
		Span<Double2> span = stackalloc Double2[resolution];
		int ellipseArray_Optimized = GetEllipseArray_Optimized(ref span, resolution);
		int num3;
		int num5;
		int num6;
		if (direction > 0)
		{
			num3 = -(ellipseArray_Optimized - 1);
			double num4 = 0.0 - span[Math.Abs(num3)].y;
			while (num4 < num && num3 < ellipseArray_Optimized - 1)
			{
				num3++;
				num4 = (double)Math.Sign(num3) * span[Math.Abs(num3)].y;
			}
			if (flag)
			{
				num5 = num3;
				num6 = 2 * ellipseArray_Optimized + 1;
			}
			else
			{
				num5 = -(ellipseArray_Optimized - 1);
				num4 = 0.0 - span[Math.Abs(num5)].y;
				while (num4 < num2 && num5 < ellipseArray_Optimized - 1)
				{
					num5++;
					num4 = (double)Math.Sign(num5) * span[Math.Abs(num5)].y;
				}
				num6 = num5 - num3;
				if (num6 < 0)
				{
		...
		{
			num3 = ellipseArray_Optimized - 1;
			double num4 = span[Math.Abs(num3)].y;
			while (num4 > num && num3 > -(ellipseArray_Optimized - 1))
			{
				num3--;
				num4 = (double)Math.Sign(num3) * span[Math.Abs(num3)].y;
		...
		if (!ellipseCache.TryGetValue(num6, out var value))
		{
			value = new Vector3[num6];
			ellipseCache.Add(num6, value);
		}
		Vector3[] array = value;
		int num7 = 0;
		double radiusAtTrueAnomaly = Kepler.GetRadiusAtTrueAnomaly(slr, ecc, num);
		...
		{
			flag2 = false;
			double num4 = (double)Math.Sign(num8) * span[Math.Abs(num8)].y;
			radiusAtTrueAnomaly = span[Math.Abs(num8)].x;
			array[num7] = Kepler.GetPosition(radiusAtTrueAnomaly, num4, arg) * scaleMultiplier;
			num7++;
			if (direction > 0)
			{
				num8++;
				if (num8 == ellipseArray_Optimized)
				{
					num8 = -(ellipseArray_Optimized - 1);
				}
			}
			else
			{
	...
	public int GetEllipseArray_Optimized(ref Span<Double2> span, int resolution)
	{
		double num = Math.PI * 2.0 / (double)resolution;
		Span<Double2> span2 = stackalloc Double2[resolution];
		int num2 = 0;
		double num3 = Math.PI;
		double radiusAtTrueAnomaly = Kepler.GetRadiusAtTrueAnomaly(slr, ecc, num3);
		span2[num2] = new Double2(radiusAtTrueAnomaly, num3);
		num2++;
		num3 -= Math.Max(2.0 - radiusAtTrueAnomaly / sma, 1E-06) * num;
		while (num3 > 0.0 && num2 < resolution - 1)
		{
		...
		span2[num2] = new Double2(periapsis, 0.0);
		num2++;
		for (int i = 0; i < num2; i++)
		{
			span[i] = span2[num2 - i - 1];
		}
		return num2;
	}
