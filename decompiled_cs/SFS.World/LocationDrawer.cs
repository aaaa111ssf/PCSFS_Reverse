...
public class LocationDrawer : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else
		{
			...
			double terrainHeight = location.GetTerrainHeight(clampToWater: true);
			text2 = ((!(terrainHeight < 2000.0) && !(location.Height < 500.0)) ? ((string)(flag ? Loc.main.Height_Vertical : Loc.main.Height_Horizontal).Inject(location.Height.ToDistanceString(), "height")) : ((string)(flag ? Loc.main.Height_Terrain_Vertical : Loc.main.Height_Terrain_Horizontal).Inject(terrainHeight.ToDistanceString(), "height")));
		}
	...
	private static void GetIdealAngle(out bool showAngle, out double angle, out double targetAngle)
	{
		...
		if (!(num > orbit.Planet.data.atmospherePhysics.height) && !(rocket.location.velocity.Value.Rotate(0.0 - rocket.location.position.Value.AngleRadians + Math.PI / 2.0).y < 10.0) && !(rocket.location.Value.Height < 10.0))
		{
			showAngle = true;
