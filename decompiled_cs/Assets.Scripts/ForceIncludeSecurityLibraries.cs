...
public class ForceIncludeSecurityLibraries : MonoBehaviour
{
	private void Start()
	{
		try
		{
			...
			new Thread((ParameterizedThreadStart)null).Start(base.name);
		}
		catch
		{
		}
	...
	private void Test()
	{
		Debug.Log(typeof(AndroidJavaClass).AssemblyQualifiedName);
		Debug.Log(typeof(AndroidJavaObject).AssemblyQualifiedName);
		Debug.Log(typeof(AndroidJNI).AssemblyQualifiedName);
		Debug.Log(typeof(ZipEntry).AssemblyQualifiedName);
		Debug.Log(typeof(ZipFile).AssemblyQualifiedName);
		Debug.Log(typeof(SHA1Managed).AssemblyQualifiedName);
		Debug.Log(typeof(Thread).AssemblyQualifiedName);
		Debug.Log(typeof(Encoding).AssemblyQualifiedName);
	}
}
