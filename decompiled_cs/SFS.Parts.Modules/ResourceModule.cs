...
public class ResourceModule : MonoBehaviour, I_InitializePartModule, Rocket.INJ_Rocket, ResourceDrawer.I_Resource
{
	...
	public void Draw(List<ResourceModule> modules, StatsMenu drawer, PartDrawSettings settings)
	{
		...
		{
			return Loc.main.Info_Resource_Amount.InjectField(resourceType.displayName, "resource", formatCapitalization: true).Inject(ResourceAmount.ToString(2, forceDecimals: false) + resourceType.resourceUnit.Field, "amount");
		}
		string MaxSize()
		{
			return Loc.main.Info_Resource_Amount.InjectField(resourceType.displayName, "resource", formatCapitalization: true).Inject(TotalResourceCapacity.ToString(2, forceDecimals: false) + resourceType.resourceUnit.Field, "amount");
		}
	...
	private void RecalculateMass()
	{
		...
		if (setDensity)
		{
			part.density.Value = Mathf.Lerp((float)DryMassPercent, 1f, (float)resourcePercent.Value) * resourceType.density;
		}
	}
	...
	public static ResourceModule CreateGroup(List<ResourceModule> resources, GameObject holder)
	{
		...
		resourceModule.setDensity = false;
