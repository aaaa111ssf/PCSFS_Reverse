...
public class ParachuteModule : MonoBehaviour, Rocket.INJ_Location, I_PartMenu
{
	...
	public AudioModule deploySound_Partial;

	public AudioModule deploySound_Fully;
	...
	public void DeployParachute(UsePartData data)
	{
		...
		if (targetState.Value == 0f && state.Value == 0f)
		{
			...
			else if (Location.GetTerrainHeight(clampToWater: true) > maxDeployHeight * num)
			{
			...
			else if (Location.planet.data.hasWater && WorldView.ToGlobalPosition(parachute.position).magnitude < Location.planet.Radius)
			{
		...
		{
			if (Location.GetTerrainHeight(clampToWater: true) > maxDeployHeight * 0.2 * num)
			{
	...
	private void LateUpdate()
	{
		...
		AngleToOldPosition();
		if (targetState.Value == 1f && Location.GetTerrainHeight(clampToWater: true) < maxDeployHeight * 0.2 * Location.planet.data.atmospherePhysics.parachuteMultiplier - 300.0)
		{
			targetState.Value = 2f;
		}
		if (targetState.Value == 2f && (Location.velocity.Mag_LessThan(0.009999999776482582) || (Location.planet.data.hasWater && WorldView.ToGlobalPosition(parachute.position).magnitude < Location.planet.Radius)))
		{
			targetState.Value = 3f;
			state.Value = 3f;
		}
