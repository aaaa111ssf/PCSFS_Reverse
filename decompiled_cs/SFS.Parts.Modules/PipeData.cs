...
public abstract class PipeData : PolygonData
{
	...
	protected void SetData(Pipe pipe)
	{
		...
		{
			SetData(new Polygon(this, array), new Polygon(this, ToFastPoints(array, 0.05f)));
		}
		else
		{
			SetData(new Polygon(this, array));
		}
	}
	...
	public override bool Raycast(Object debugObject, Vector2 point, out float depth)
	{
		...
		{
			if (Depth(debugObject, pipe.points[i], pipe.points[i + 1], point, out var depth2))
			{
	...
	private static bool Depth(Object debugObject, PipePoint p0, PipePoint p1, Vector2 point, out float depth)
	{
		return Depth(debugObject, p1.position, p1.width, p0.position, p0.width, point, out depth);
	}

	private static bool Depth(Object debugObject, Vector2 bottom, Vector2 bottomWidth, Vector2 top, Vector2 topWidth, Vector2 point, out float depth)
	{
		depth = 0f;
		...
		if (new Polygon(debugObject, vector, vector2, vector3, vector4).convexPolygons[0].GetDistanceToPolygon(point) > 0f)
		{
			return false;
		}
