...
public class BurnMark : MonoBehaviour
{
	...
	public void SetBurn(Vector2 velocityDirection, Transform positionContext, float intensity, Line2[] topSurfaces_World, Line2[] bottomSurfaces_World, float opacity)
	{
		...
		bottomSurfaces_World = Enumerable.Reverse(bottomSurfaces_World).ToArray();
	...
	private bool CreateTexture(Bounds bounds)
	{
		...
		if (num2 == 0)
		{
			Debug.LogError("Invalid texture size 0 when applying BurnMarks for Part:" + base.name, this);
			return false;
		}
