...
public class FlowModule : MonoBehaviour
{
	...
	public class Flow
	{
		...
		public bool CanFlow_ElseShowMsg(I_MsgLogger logger)
		{
			...
			{
				logger.Log(Loc.main.Msg_No_Resource_Source.InjectField(resourceType.displayName, "resource", formatCapitalization: true));
			}
			if ((FlowState)state == FlowState.NoResource)
			{
				logger.Log(Loc.main.Msg_No_Resource_Left.InjectField(resourceType.displayName, "resource", formatCapitalization: true));
			}
