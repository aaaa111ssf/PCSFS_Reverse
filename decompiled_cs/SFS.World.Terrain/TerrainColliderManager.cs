	...
	public class Chunk
	{
		...
		public Chunk(double from, double size, Planet planet, Transform holder)
		{
			...
			if (planet.data.hasTerrain && planet.data.terrain.collider)
			{
				double[] angles;
				double[] height;
				SFS.World.Terrain.Chunk.TerrainPoints terrainPoints = planet.data.terrain.GetTerrainPoints(from, size, 11, offset: true, forCollider: true, planet, out position, out angles, out height);
				gameObject.AddComponent<PolygonCollider2D>().points = ((IEnumerable<Vector3>)terrainPoints.points).Select((Func<Vector3, Vector2>)((Vector3 a) => a)).ToArray();
			}
