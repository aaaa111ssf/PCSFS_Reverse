...
public class DynamicTerrain : MonoBehaviour
{
	...
	public class DynamicChunk
	{
		...
		private void GenerateRocks()
		{
			...
			double[] terrainHeightAtAngles = planet.GetTerrainHeightAtAngles(array, clampToWater: false);
			...
			for (int num5 = 0; num5 < list.Count; num5++)
			{
				...
				Transform transform = UnityEngine.Object.Instantiate(owner.rockPrefab, chunk.terrainTransform);
				rocks.Add(transform);
				Double2 @double = Double2.CosSin(tuple.Item1, planet.Radius + terrainHeightAtAngles[num5]);
				Vector2 vector = @double - chunk.terrainTransform.localPosition;
		...
		public bool CanMerge()
		{
			...
			{
				return otherHalf.chunk.terrainTransform.gameObject.activeSelf;
			}
			return false;
		}
		...
		public void EnableChunk()
		{
			...
			chunk.terrainTransform.gameObject.SetActive(value: true);
		...
		public void DisableChunk()
		{
			...
			chunk.terrainTransform.gameObject.SetActive(value: false);
			foreach (Transform rock in rocks)
			{
		...
		public void DestroyChunk()
		{
			...
			owner.activeChunks.Remove(this);
			UnityEngine.Object.Destroy(chunk.terrainTransform.gameObject);
			UnityEngine.Object.DestroyImmediate(chunk.terrainMesh);
			UnityEngine.Object.DestroyImmediate(chunk.waterMesh);
			foreach (Transform rock in rocks)
			{
				RockSelector.main.rockInstances.Remove(rock);
			}
		...
		{
			chunk.terrainTransform.gameObject.layer = layer;
			if (owner.planet.data.hasWater)
			{
				chunk.waterTransform.gameObject.layer = layer;
			}
		}
	}
	...
	public static DynamicTerrain Create(Planet planet, Double3 position, Action<DynamicChunk> setupChunk, float verticeSizeMultiplier, float loadDistanceMultiplier, string layer, (Material, Material) material, bool useTerrainUV, Transform chunkPrefab, TerrainModule.RockData rockData, Transform rockPrefab)
	{
	...
	{
		if (bestSplit == null || bestSplit.chunk.terrainTransform == null)
		{
			CalculateBest();
		}
		return bestSplit;
	...
	{
		if (bestMerge == null || bestMerge.chunk.terrainTransform == null)
		{
			CalculateBest();
		}
		return bestMerge;
