...
public abstract class AeroModule : MonoBehaviour
{
	...
	public static List<Surface> GetExposedSurfaces(List<Surface> surfacesList)
	{
		...
		Surface[] array = Sort(surfacesList);
		List<Surface> output = new List<Surface> { array.First() };
		for (int i = 1; i < array.Length; i++)
		{
			Surface surface = array[i];
	...
	public static Surface[] Sort(List<Surface> list)
	{
		...
		{
			uint num = BitConverter.ToUInt32(BitConverter.GetBytes(list[i].line.end.x), 0);
			array[i] = (((num & 0x80000000u) != 0) ? (~num) : (num ^ 0x80000000u));
		}
		Surface[] array2 = list.ToArray();
		RadixSort(array, array2);
		return array2;
	}
	...
	private static void RadixSort(uint[] keys, Surface[] values)
	{
		...
		Surface[] array2 = new Surface[num];
		for (int i = 0; i < 32; i += 8)
		{
			int[] array3 = new int[256];
			for (int j = 0; j < num; j++)
			{
				array3[(keys[j] >> i) & 0xFF]++;
			}
			for (int k = 1; k < 256; k++)
			{
				array3[k] += array3[k - 1];
			...
			{
				int num3 = (int)((keys[num2] >> i) & 0xFF);
				int num4 = --array3[num3];
				array[num4] = keys[num2];
				array2[num4] = values[num2];
			}
			Array.Copy(array, keys, num);
			Array.Copy(array2, values, num);
		}
	}
