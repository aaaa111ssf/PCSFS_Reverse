...
public class PartsLoader : MonoBehaviour
{
	...
	private void Awake()
	{
		colorTextures = ResourcesLoader.GetFiles_Dictionary<ColorTexture>("Part Textures");
		shapeTextures = ResourcesLoader.GetFiles_Dictionary<ShapeTexture>("Part Textures");
		...
		parts = item;
		partVariants = item2;
		CustomAssetsLoader.LoadAllCustomAssets();
	}
	...
	public static Part CreatePart(PartSave partSave, Transform holder, string sortingLayer, OnPartNotOwned onPartNotOwned, out OwnershipState ownershipState)
	{
		...
		{
			if (partSave.burns != null && partSave.burns.intensity > 0f)
			{
