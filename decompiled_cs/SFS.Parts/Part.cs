...
public class Part : HeatModuleBase, Rocket.INJ_Rocket
{
	...
	public TranslationVariable pickCategoryName = new TranslationVariable();
	...
	public Float_Local density = new Float_Local(0.5f);
	...
	[NonSerialized]
	public int frameIndex_WaterDamage = -10;
	...
	public void DrawPartStats(Part[] allParts, StatsMenu drawer, PartDrawSettings settings)
	{
		...
		drawer.DrawStat(100, () => Loc.main.Density.Inject(density.Value.ToString(1, forceDecimals: true), "value"), null, delegate(Action update)
		{
			density.OnChange += update;
		}, delegate(Action update)
		{
			density.OnChange -= update;
		});
