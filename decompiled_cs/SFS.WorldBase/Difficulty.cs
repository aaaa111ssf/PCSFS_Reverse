...
public class Difficulty
{
	...
	public void ScalePlanetData(PlanetData planet)
	{
		...
		if (planet.hasAtmosphereVisuals)
		{
			...
			atmosphereVisuals.GRADIENT.height *= num2;
			atmosphereVisuals.GRADIENT.positionZ = (int)((float)atmosphereVisuals.GRADIENT.positionZ * num2);
			atmosphereVisuals.CLOUDS.startHeight *= num2;
			atmosphereVisuals.CLOUDS.width *= num2;
			atmosphereVisuals.CLOUDS.height *= num2;
			Atmosphere_Visuals.ColorGradient.Key[] keys = atmosphereVisuals.FOG.keys;
			for (int i = 0; i < keys.Length; i++)
			{
				keys[i].distance *= num2;
			}
		}
		if (planet.hasPostProcessing)
		{
			PostProcessingModule.Key[] keys2 = planet.postProcessing.keys;
		...
		{
			FrontCloudsModule frontClouds = planet.frontClouds;
			frontClouds.positionZ *= num2;
			frontClouds.height *= num2;
			frontClouds.fadeZoneHeight *= num2;
		}
		if (planet.hasRings)
		{
			RingsModule rings = planet.rings;
			rings.startRadius *= num;
			rings.endRadius *= num;
			rings.positionZ *= (float)num;
		}
		if (planet.hasOrbit)
		{
	...
	{
		if (!planet.hasAtmospherePhysics || !planet.atmospherePhysics.heightDifficultyScale.TryGetValue(difficulty, out var value))
		{
			if (!planet.hasAtmosphereVisuals || !planet.atmosphereVisuals.GRADIENT.heightDifficultyScale.TryGetValue(difficulty, out var value2))
			{
