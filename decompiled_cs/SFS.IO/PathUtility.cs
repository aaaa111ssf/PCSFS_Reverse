...
public static class PathUtility
{
	...
	public static string AutoNameExisting(string name, IFolder path)
	{
		name = FilePath.CleanupName(name);
		List<string> list = new List<string>();
		IStorageObject[] children = path.GetChildren();
		foreach (IStorageObject storageObject in children)
		{
			if (!(storageObject is IFile file))
			{
				if (storageObject is IFolder folder)
				{
