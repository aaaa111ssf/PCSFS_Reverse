...
public class Console : Screen_Menu
{
	...
	private void Awake()
	{
		...
		commands.Add(delegate(string s)
		{
			...
			IFile file = FileLocations.GetBaseFolder().GetFolder("Logs").GetFile(match.Groups[1].Value);
			file.WriteText(string.Join(Environment.NewLine, main.queue));
			main.WriteText($"Successfully saved log to {file}");
			return true;
		});
