...
public class MapManager : MonoBehaviour
{
	...
	private static void DrawTrajectories()
	{
		...
		foreach (Planet planet in Base.planetLoader.planets.Values)
		{
			if (planet.HasParent)
			{
				...
				{
					float num = (planet.data.basics.significant ? 1f : Mathf.Lerp(0.3f, 1f, MapDrawer.GetFadeOut(Map.view.view.distance, planet.Radius * 30000.0, planet.Radius * 30000.0 * 1.5)));
					Color mapColor = planet.data.basics.mapColor;
					mapColor.a = num * 2.5f;
					planet.orbit.DrawOrbit(mapColor, drawStats: false, drawStartText: false, drawEndText: false, Map.solidLine);
				};
			}
		}
		...
		if (Map.navigation.target != null && !(Map.navigation.target is MapPlanet))
		{
	...
	private static void DrawPlanetName(Vector2 normal, Planet planet, bool selected, bool target)
	{
		...
		Map.elementDrawer.DrawTextElement(planet.DisplayName, normal, 40, planet.data.basics.mapColor * new Color(1f, 1f, 1f, num), planet.mapHolder.position, priority, clearBelow: true, 2);
	}
	...
	private static void DrawPlanetDots()
	{
		foreach (Planet value in Base.planetLoader.planets.Values)
		{
			...
			if (num > 0f)
			{
				Map.elementDrawer.DrawTextElement("●", Vector2.zero, 35, value.data.basics.mapColor * new Color(1f, 1f, 1f, num), value.mapHolder.position, -1, clearBelow: false, 1);
			}
		}
	}
	...
	private static void DrawLandmarks()
	{
		foreach (Planet value in Base.planetLoader.planets.Values)
		{
			...
			foreach (Landmark landmark in landmarks)
			{
				...
				float num2 = Mathf.Min(MapDrawer.GetFadeIn(Map.view.view.distance, num * 0.5, num * 0.4));
				if (!(num2 <= 0f))
				{
					Color color = new Color(1f, 1f, 1f, num2);
					Vector2 normal = Double2.CosSin(MathF.PI / 180f * landmark.data.Center);
