...
public class MapEnvironment : MonoBehaviour
{
	...
	private MapPlanetEnvironment CreateEnvironment(Planet planet, Double3 cameraPosition)
	{
		...
		if (planet.HasRings)
		{
			...
			{
				color = planet.data.rings.mapColor
			};
			Rings rings = Rings.Create(planet, mapPlanetEnvironment.holder, GameManager.main.environment.atmospherePrefab, material);
			rings.SetLayer("Map");
			rings.transform.localPosition = Vector3.zero;
			rings.transform.localScale = Vector3.one / 1000f;
		}
	...
	private DynamicTerrain CreateTerrain(Planet planet, Double3 positionToCamera, Transform parent)
	{
		...
		Material item = new Material(terrainShader)
		{
			color = planet.data.basics.mapColor
		};
		Material item2 = (planet.data.hasWater ? new Material(terrainShader)
		{
			color = planet.data.water.mapColor
		} : null);
		DynamicTerrain dynamicTerrain = DynamicTerrain.Create(planet, new Double3(0.0 - positionToCamera.x, 0.0 - positionToCamera.y, positionToCamera.z), SetupChunk, 4f, 0.6f, "Map", (item, item2), useTerrainUV: false, chunkPrefab, null, null);
	...
	{
		MeshRenderer component = a.chunk.terrainTransform.GetComponent<MeshRenderer>();
