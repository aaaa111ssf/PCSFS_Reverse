...
public class TimewarpTo : MonoBehaviour
{
	...
	private static bool GetPosition(Select_Point a, out Vector3 position)
	{
		if (a != null)
		{
			...
			{
				if (path is Orbit orbit && orbit.Planet == a.planet && TryGetRadiusFromAngle(orbit, a.angleRadians, out var radius) && (double)(radius * 1000f) > a.planet.Radius + a.planet.GetTerrainHeightAtAngle(a.angleRadians, clampToWater: true))
				{
	...
	{
		if (!WorldTime.CanTimewarp(showMsg: true, showSpeed: false, out var _))
		{
			return;
		}
		Select obj = selected;
